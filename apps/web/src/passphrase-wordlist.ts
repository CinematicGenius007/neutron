export const PASSPHRASE_WORDLIST_COUNT = 7_776;
export const PASSPHRASE_WORDLIST_SHA256 =
  "abae49761b88f3f1ba31ef944bea1f61b795a3cd7e1cfb7d276ed45bf77967ba";

export const EFF_LONG_WORDLIST_ATTRIBUTION =
...
export function isPassphraseWord(candidate: string): boolean {
  let lower = 0;
  let upper = PASSPHRASE_WORDLIST.length;
  while (lower < upper) {
    const middle = lower + Math.floor((upper - lower) / 2);
    const word = PASSPHRASE_WORDLIST[middle];
    if (word === undefined) return false;
    if (word === candidate) return true;
    if (word < candidate) lower = middle + 1;
    else upper = middle;
  }
  return false;
}
